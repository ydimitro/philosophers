...
/*   Updated: 2023/05/14 23:31:46 by ydimitro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_philo	*create_all_philos(t_input *input, t_mutex *death, t_mutex *print, t_mutex *cycles)
{
	int		i;
	t_philo	*storage;
	t_philo	*last;
	...
	while (i < input->nbr_philos)
	{
		...
		last = last->next; //last should allows point to the last element
		last->pid = i;
		last->cycles_fullfilled = cycles;
		i++;
	}
